...
SCRIPT = Path(__file__).resolve().parent.parent / "skills" / "wiki-lint" / "scripts" / "check_evidence.py"
...
BODY_BLOCKQUOTE_ARTICLE = """# Ordinary article

> Sources: Example, 2026-01-01

Some paragraph first.

> Note: superseded on 2025-01-01
...
class HeaderScopeTest(WikiTestCase):
    def test_body_blockquote_does_not_create_header(self):
        plain_wiki(self.root, "a.md", BODY_BLOCKQUOTE_ARTICLE)
        result = run_checker(self.root)
        self.assertIn("no Raw field", result.stdout)
...
class DocumentStructureTest(WikiTestCase):
    ...
    def test_fence_before_real_h1_does_not_create_header(self):
        article = (
            "```markdown\n# Fake\n> Type: Entity\n```\n\n"
            "# Real article\n\n> Sources: Example, 2026-01-01\n\nValue 777K.\n"
        )
...
class ExamplesSmokeTest(WikiTestCase):
    def test_examples_have_zero_suspects(self):
        result = run_checker(EXAMPLES_DIR / "demo-wiki")
        self.assertIn("0 fidelity suspect(s)", result.stdout)
        self.assertIn("0 evidence error(s)", result.stdout)
        self.assertIn("0 unreferenced raw file(s)", result.stdout)
        self.assertIn("0 modified raw file(s)", result.stdout)
        self.assertIn("0 page(s) behind evidence", result.stdout)


class IntegrityTest(WikiTestCase):
    def setUp(self):
        super().setUp()
        plain_wiki(self.root, "a.md", PLAIN_ARTICLE, raw_name="plain.md")

    def hashed_log(self) -> str:
        raw_path = self.root / "raw" / "t" / "plain.md"
        digest = hashlib.sha256(raw_path.read_bytes()).hexdigest()
        return (
            "# Wiki Log\n\n"
            "## [2026-06-01] ingest | Source\n"
            "- Disposition: New\n"
            "- Raw: raw/t/plain.md\n"
            f"- SHA-256: {digest}\n"
        )
    ...
    def test_matching_hash_passes(self):
        (self.root / "wiki" / "log.md").write_text(self.hashed_log())
        result = run_checker(self.root)
        self.assertIn("0 modified raw file(s)", result.stdout)
        self.assertNotIn("- raw/t/plain.md", result.stdout)

    def test_modified_raw_is_flagged(self):
        (self.root / "wiki" / "log.md").write_text(self.hashed_log())
        (self.root / "raw" / "t" / "plain.md").write_text(PLAIN_RAW + "Sneaky edit.\n")
        result = run_checker(self.root)
        self.assertIn("raw file modified since ingest: raw/t/plain.md", result.stdout)
        self.assertIn("1 modified raw file(s)", result.stdout)

    def test_unhashed_raw_is_informational_not_strict(self):
        article = (
            "# A\n\n> Sources: Example, 2026-01-01\n"
            "> Raw: [plain](../../raw/t/plain.md)\n\nNo numeric claims at all.\n"
        )
        (self.root / "wiki" / "t" / "a.md").write_text(article)
        result = run_checker(self.root, "--strict")
        self.assertEqual(result.returncode, 0)
        self.assertIn("- raw/t/plain.md", result.stdout)
        self.assertNotIn("raw file modified since ingest", result.stdout)


class FreshnessTest(WikiTestCase):
    def setUp(self):
        super().setUp()
        plain_wiki(self.root, "a.md", PLAIN_ARTICLE, raw_name="plain.md")
        digest = hashlib.sha256((self.root / "raw" / "t" / "plain.md").read_bytes()).hexdigest()
        self.log = (
            "# Wiki Log\n\n"
            "## [2026-06-01] ingest | Source\n"
            "- Disposition: New\n"
    ...
    def article_with_updated(self, updated: str) -> None:
        article = PLAIN_ARTICLE.replace(
            "> Raw: [plain](../../raw/t/plain.md)",
            f"> Raw: [plain](../../raw/t/plain.md)\n> Updated: {updated}",
        )
        (self.root / "wiki" / "t" / "a.md").write_text(article)

    def test_page_updated_on_or_after_latest_ingest_passes(self):
        self.article_with_updated("2026-06-01")
        (self.root / "wiki" / "log.md").write_text(self.log)
        result = run_checker(self.root)
        self.assertIn("0 page(s) behind evidence", result.stdout)

    def test_page_older_than_latest_ingest_is_flagged(self):
        self.article_with_updated("2026-05-01")
        (self.root / "wiki" / "log.md").write_text(self.log)
        result = run_checker(self.root)
        self.assertIn("predates the latest ingest (2026-06-01)", result.stdout)
        self.assertIn("1 page(s) behind evidence", result.stdout)

    def test_page_without_updated_is_skipped(self):
        (self.root / "wiki" / "log.md").write_text(self.log)
        result = run_checker(self.root)
        self.assertIn("0 page(s) behind evidence", result.stdout)

    def test_latest_ingest_across_entries_wins(self):
        self.article_with_updated("2026-05-15")
        digest = hashlib.sha256((self.root / "raw" / "t" / "plain.md").read_bytes()).hexdigest()
        log = (
            "# Wiki Log\n\n"
            "## [2026-04-01] ingest | Source\n"
            "- Disposition: New\n"
            "- Raw: raw/t/plain.md\n"
            f"- SHA-256: {digest}\n\n"
            "## [2026-06-01] ingest | Source\n"
